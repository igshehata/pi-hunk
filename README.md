...
By default, Hunk opens in the right half of the terminal and Pi always wraps into the left half for
`left`/`right` layouts. Available layouts are `full`, `left`, `right`, and `float`. Host mode is
